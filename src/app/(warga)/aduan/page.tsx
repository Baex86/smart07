...
export default function AduanWargaPage() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    const res = await getTiketWarga('aduan');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await buatTiket('aduan', formData);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 pb-24 md:pb-8">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-extrabold text-navy-900 tracking-tight">Aduan Warga</h1>
          <p className="text-navy-500 mt-1 text-sm font-medium">Layanan tiket keluhan privat ke Admin RT.</p>
        </div>
        {!showForm && (
      ...
      {showForm && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="bg-white p-6 rounded-2xl border border-ivory-300 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-bold text-navy-900 flex items-center gap-2"><MessageSquareWarning size={18} className="text-red-500" /> Tulis Laporan Aduan</h3>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={isSubmitting} className="w-full py-3 bg-navy-900 text-white font-bold rounded-xl hover:bg-navy-800 transition flex justify-center items-center gap-2 shadow-md disabled:opacity-70">
              {isSubmitting ? <Loader2 size={18} className="animate-spin" /> : <Send size={18} />} Kirim Aduan
            </button>
          </form>
        </motion.div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="bg-ivory-50 border-2 border-dashed border-ivory-300 rounded-2xl p-8 text-center">
          ...
          <p className="font-bold text-navy-900">Belum Ada Aduan</p>
          <p className="text-sm text-navy-400 mt-1">Anda belum pernah mengirim aduan.</p>
        </div>
      ) : (
